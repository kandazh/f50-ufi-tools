...
socat_guard_once() {
    ...
    # Kill expired child socat processes first
    for _pid in $_expired_child_pids; do
        [ "$_pid" = "$_oldest_pid" ] && continue
        _sg_kill_one "$_pid"
    done

    # Recount to avoid using stale count after killing above
    _count2=0
    for _d in /proc/[0-9]*; do
        [ -r "$_d/comm" ] || continue
        IFS= read -r _name < "$_d/comm" || continue
    ...
    # If still over limit, kill 1 more
...
schedule_script() {
  ...
  check_log_file
  keep_ufi_running 0
  # DISABLED: stable daemon started at boot, re-enable if OOM-kills observed
  check_ttyd_running
  check_socat_running

package com.hotbox.f50_app.modules.scheduledTask

import android.content.Context
import com.hotbox.f50_app.modules.BASE_TAG
import com.hotbox.f50_app.utils.HotboxLog
import com.hotbox.f50_app.utils.ShellHotbox.Companion.PREFS_NAME
...
fun Route.scheduledTaskModule(context: Context) {
    ...
    post("/api/add_task") {
        try {
            ...
            HotboxLog.d(TAG, "Failed to add task: ${e.message}")
            call.response.headers.append("Access-Control-Allow-Origin", "*")
            call.respondText(
    ...
    post("/api/remove_task") {
        try {
            ...
            HotboxLog.d(TAG, "Failed to delete task: ${e.message}")
            call.response.headers.append("Access-Control-Allow-Origin", "*")
            call.respondText(
    ...
    post("/api/clear_task") {
        try {
            ...
            HotboxLog.d("[$BASE_TAG]_pluginsModule", "Error clearing scheduled tasks: ${e.message}")
            call.response.headers.append("Access-Control-Allow-Origin", "*")
            call.respondText(
    ...
    get("/api/list_tasks") {
        try {
            ...
            HotboxLog.d(TAG, "Failed to get task list: ${e.message}")
            call.response.headers.append("Access-Control-Allow-Origin", "*")
            call.respondText(
    ...
    get("/api/get_task") {
        ...
        try {
            ...
            HotboxLog.d(TAG, "Failed to get task: ${e.message}")
            call.response.headers.append("Access-Control-Allow-Origin", "*")
            call.respondText(

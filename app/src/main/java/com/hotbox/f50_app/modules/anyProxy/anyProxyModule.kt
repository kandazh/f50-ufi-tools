package com.hotbox.f50_app.modules.at
    ...
    route("/api/proxy/{...}") {
        handle {
            ...
            for ((key, values) in call.request.headers.entries()) {
                if (key.startsWith("hotbox-", ignoreCase = true)) {
                    HotboxLog.d(TAG,"Proxy header detected $key=$values，prefix removed")
                    if(key.contains("hotbox-cookie", ignoreCase = true)) {
                        headersBuilder.add("Cookie", values.first())
                    }else {
                        headersBuilder.addUnsafeNonAscii(key.removePrefix("hotbox-"), values.first())
                    }
            ...
            try {
                ...
                response.headers.names().forEach { name ->
                    ...
                    if (lowerName == "set-cookie") {
                        values.forEach { rawCookie ->
                            call.response.headers.append("Hotbox-SetCk", rawCookie)
                            call.response.headers.append("Hotbox-Set-Cookie", rawCookie)
                        }
                ...
                if (origin != "*") {
                    ...
                    call.response.headers.append("Access-Control-Expose-Headers", "Hotbox-SetCk")
                }

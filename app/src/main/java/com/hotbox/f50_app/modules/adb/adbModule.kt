package com.hotbox.f50_app.modules.adb

import android.content.Context
import com.hotbox.f50_app.ADBService
...
import com.hotbox.f50_app.utils.ShellHotbox.Companion
...
fun Route.adbModule(context: Context) {
    ...
    post("/api/update_admin_pwd"){
        try {
            ...
            HotboxLog.d(TAG, "Parse ADB_WIFI POST request error: ${e.message}")
            call.response.headers.append("Access-Control-Allow-Origin", "*")
            call.respondText(
    ...
    get("/api/adb_wifi_setting") {
        try {
            ...
            HotboxLog.d(TAG, "Error getting network ADB info: ${e.message}")
    ...
    post("/api/adb_wifi_setting") {
        try {
            ...
            HotboxLog.d(TAG, "ADMIN_PWD:${sharedPrefs.getString("ADMIN_PWD", "")}")

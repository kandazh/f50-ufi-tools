...
fun Route.reverseProxyModule(targetServerIP:String) {
    ...
    route("/api/goform/{...}") {
        HotboxLog.d(TAG,"Starting reverse proxy...")
        handle {
            ...
            try {
                ...
                conn = (url.openConnection() as HttpURLConnection).apply {
                    ...
                    val ck = call.request.headers["Hotbox-Cookie"]
                    if(!ck.isNullOrBlank()) {
                ...
                conn.headerFields.forEach { (key, values) ->
                    if (key != null && key.equals("Set-Cookie", ignoreCase = true)) {
                        values?.forEach { cookie ->
                            call.response.headers.append("hotbox-cookie", cookie)
                        }
                    }
                }
                ...
                HotboxLog.e(TAG,"Forward error",e)

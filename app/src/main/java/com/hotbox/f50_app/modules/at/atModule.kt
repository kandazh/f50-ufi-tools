...
fun Route.atModule(context: Context) {
    ...
    get("/api/AT") {
        try {
            ...
            HotboxLog.d(TAG, "AT_command parameter: $command")
            ...
            val outFileAt = cachedSendatFile?.takeIf { it.exists() && it.canExecute() }
                ?: run {
                    val f = HotboxUtils.copyFileToFilesDir(context, "shell/sendat")
            ...
            val result = ShellHotbox.runShellCommand(atCommand, true)
            ...
            HotboxLog.d(TAG, "AT_cmd：$atCommand")
            HotboxLog.d(TAG, "AT_result：$res")
            ...
            HotboxLog.d(TAG, "AT commandExecution error: ${e.message}")
